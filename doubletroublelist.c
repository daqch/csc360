...
void swap(struct node *ref1, struct node *ref2)
{
    ...
    /*nodes are tail and head of the list
    */
    if (ref1->prev == ref2 || ref1->next == ref2)
    {
        if (ref1->prev == ref2)
        {

            struct node *prev = ref2->prev;
            struct node *next = ref1->next;
            ref1->next = ref2;
            ref2->prev = ref1;
        ...
        {
            struct node *prev = ref1->prev;
            struct node *next = ref2->next;
...
void reverse(struct node *ref)
{
    ...
    while (head != tail)
    { // Stop condition for list with odd elements
        ...
        if (tail == nx)
        { // Stop condition for list with even elements
            break;
        }
...
{
    struct node *l1_end = end(ref1);
    struct node *l2_head = begin(ref2);
    l1_end->next = l2_head;
    l2_head->prev = l1_end;
...
void printLinks(struct node *ref)
{
    struct node *n = begin(ref);
    do
    {
        ...
        {
            printf("head or tail value:  %d", n->data);
            fflush(stdout);
            if (n->prev == NULL)
            {
                printf(" has %d as next \n", n->next->data);
                fflush(stdout);
            }
            else
                printf(" has %d as prev\n", n->prev->data);
            fflush(stdout);
        }
        else
        {
            printf("%d  ", n->data);
            fflush(stdout);
            printf("has %d has prev and %d as next\n", n->prev->data, n->next->data);
            fflush(stdout);
        }
    } while ((n = next(n)) != NULL);
    printf("\n");
